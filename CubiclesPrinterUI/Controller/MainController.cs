    ...
    public sealed class MainController : IDisposable
    {
        ...
        private void InitTimer()
        {
            try
            {
                ...
                _timerRetry = new Timer()
                {
                    Enabled = false,
                    AutoReset = false,
                    Interval = ConfigData.Interval_TimerRestart                    
                };
            ...
            try
            {
                ...
                _timerGetPrinters = new Timer()
                {
                    Enabled = false,
                    AutoReset = false,
                    Interval = ConfigData.Interval_TimerGetPrinters
                };
        ...
        private void Init()
        {
            try
            {
                ...
                localPrintEventWatcher.AllowedTitleAdded += LocalPrintEventWatcher_AllowedTitleAdded;

                // set up allowed printers to skip
                localPrintEventWatcher.AllowedPrintersToSkip = allp;
        ...
        public void ShowProgressWindow(string title, string message)
        {
            _dispatcher.BeginInvoke(new Action(() =>
            {
                // close progress window if opened
                CloseProgressWindow();
                ...
                // set topmost and show
                _progressWindow.Topmost = true;
                _progressWindow.Show();

                SetProgressWindowTopMost();

            }), DispatcherPriority.Background);            
        }

        /// <summary>
        /// Sets progress window topmost.
        /// </summary>
        public void SetProgressWindowTopMost()
        {
        ...
        {
            LogHelper.LogDebug("Closing ProgressWindow");
            if (_progressWindow != null)
